    ...
    useEffect(() => {
        async function fetchData() {
            try {
                ...
                if (Array.isArray(wData) && wData.length > 0) setWorksList(wData);
                if (mData && mData.personal) setConfig(mData);
            } catch (err) {
                console.error('Failed to fetch selected works data:', err);
            }
        }
        fetchData();
    }, []);
        ...
        <section className={styles.section} id={'works'} ref={container}>
            ...
            <div className={styles.xScrollContainer} ref={galleryContainer}>
                <header className={styles.header}>
                    ...
                    <FancyButton theme='button-2' link={`mailto:${config.personal?.email || ''}`} target={'_blank'}>
                        Contact
                    </FancyButton>
                ...
                {worksList.map((work, index) => {
                    const lightness = parseFloat(work.color?.l || '50%');
